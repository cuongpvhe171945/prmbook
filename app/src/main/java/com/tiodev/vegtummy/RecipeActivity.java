...
//    String [] ingList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String[] ingList = getIntent().getStringExtra("des").split("\n");
        StringBuilder formattedIngredients = new StringBuilder();
        for (int i = 0; i < ingList.length; i++) {
            if (!ingList[i].trim().isEmpty()) {
                formattedIngredients.append("\uD83D\uDFE2  ").append(ingList[i].trim()).append("\n");
            }
        }
        ing.setText(formattedIngredients.toString());
        // Set time
